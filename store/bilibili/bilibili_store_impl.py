...
class BiliDbStoreImplement(AbstractStore):
    ...
    async def store_contact(self, contact_item: Dict):
        ...
        if not contact_detail:
            ...
            # 添加task_times_id字段
            if hasattr(config, 'TASK_ID') and config.TASK_ID:
                contact_item["task_times_id"] = config.TASK_ID
...
class BiliHybridDbStoreImplement(AbstractStore):
    ...
    def __init__(self):
        self.sqlite_store = BiliDbStoreImplement()
    ...
    async def _get_mysql_store(self):
        ...
        if self.mysql_store is None:
            ...
            # 创建MySQL存储实例
            self.mysql_store = BiliDbStoreImplement()
    ...
    async def store_contact(self, contact_item: Dict):
        """
        同时保存联系人信息到SQLite和MySQL数据库
        """
        # 保存到SQLite（默认）
        await self.sqlite_store.store_contact(contact_item)
        
        # 如果启用MySQL同步，也保存到MySQL
        if getattr(config, 'SYNC_TO_MYSQL', False):
            try:
                mysql_store = await self._get_mysql_store()
                await mysql_store.store_contact(contact_item)
            except Exception as e:
                utils.logger.warning(f"Failed to sync contact to MySQL: {e}")
    
    async def store_dynamic(self, dynamic_item: Dict):
        """
        同时保存动态信息到SQLite和MySQL数据库
        """
        # 保存到SQLite（默认）
        await self.sqlite_store.store_dynamic(dynamic_item)
        
        # 如果启用MySQL同步，也保存到MySQL
        if getattr(config, 'SYNC_TO_MYSQL', False):
            try:
                mysql_store = await self._get_mysql_store()
                await mysql_store.store_dynamic(dynamic_item)
            except Exception as e:
                utils.logger.warning(f"Failed to sync dynamic to MySQL: {e}")
